...
async fn main() -> std::io::Result<()> {
    ...
    println!("Actix QueueStorm server listening on http://0.0.0.0:{port}");

    HttpServer::new(|| {
        App::new()
            .app_data(web::JsonConfig::default().limit(8 * 1024))
            .route("/health", web::get().to(health))
            .route("/sort-ticket", web::post().to(sort_ticket_handler))
            .route("/bench/json", web::post().to(bench_json_handler))
            .route("/bench/cpu", web::post().to(bench_cpu_handler))
...
async fn sort_ticket_handler(
    payload: Result<web::Json<TicketRequest>, actix_web::Error>,
) -> impl Responder {
    ...
    HttpResponse::Ok().json(sort_ticket(
        ticket.ticket_id.as_deref().unwrap(),
        ticket.message.as_deref().unwrap(),
    ))
}

async fn bench_json_handler(payload: Result<web::Json<Value>, actix_web::Error>) -> impl Responder {
    ...
    HttpResponse::Ok().json(bench_json(&body))
}

async fn bench_cpu_handler(payload: Result<web::Json<Value>, actix_web::Error>) -> impl Responder {
    let body = match payload {
        Ok(payload) => payload.into_inner(),
        Err(_) => {
            return HttpResponse::BadRequest()
...
fn sort_ticket(ticket_id: &str, message: &str) -> TicketResponse {
    ...
    TicketResponse {
        ...
        severity,
        department: department_for(classification.case_type, severity),
